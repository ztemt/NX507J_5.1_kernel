...
static ssize_t tmg3993_get_als_value(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
    ...
    if (NULL != chip)
    {
        ...
        //taos_als_get_data();
		strcpy(buf,"1");
        schedule_delayed_work(&chip->als_poll_work, msecs_to_jiffies(200));
        return strlen(buf);
    }
    else
    {
        sprintf(buf, "tmg3993_chip is NULL\n");
    }
	return strlen(buf);
}
static ssize_t tmg3993_get_prox_value(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
    ...
    {
        SENSOR_LOG_ERROR("get_prox_value\n");
        schedule_delayed_work(&chip->prox_flush_work, msecs_to_jiffies(200));
...
static struct device_attribute attrs_prox[] = {
    ...
    __ATTR(prox_value,                      0444,   tmg3993_get_prox_value,                     NULL),
};
...
static struct device_attribute attrs_light[] = {
  ...
  __ATTR(light_value,             0644,   tmg3993_get_als_value,                  NULL),
};
...
static void tmg3993_flush_work_func(struct work_struct * work) //iVIZM
{
	tmg399x_report_prox(p_global_tmg399x_chip);
}
...
bypass_ges_idev:
    ...
    INIT_DELAYED_WORK(&chip->prox_flush_work, tmg3993_flush_work_func);
